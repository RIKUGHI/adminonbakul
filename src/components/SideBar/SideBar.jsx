          ...
          case 'Kelola':
            return(
              <ul className={`sub-menu${isOpen1 ? ' open' : ''}`}>
                <ItemSubMenu to="/pengguna" menuName="Pengguna" />
              </ul>
            )
...
const SideBar = (p) => {
  useEffect(() => {
    ...
    const menuWithSub = menu.slice(1, 4)
    // if user click menu
    menu.forEach((m, i) => {
      if (i === 1 || i === 2 || i === 3) {
        m.onclick = () => {
    ...
    <>
      <div className="side-bar" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
        <div className="head-side">
          ...
          <div className="title">
            <span className="skeleton1">{p.state.name}</span>
            <span className="skeleton1">Admin</span>
          </div>
        </div>
        <ul className="menu">
